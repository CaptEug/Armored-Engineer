...
var transparent_turrets: Array[Dictionary] = []

@export var gamemap: GameMap

var vehicle_scene: PackedScene = load("res://assembly/vehicle/Vehicle.tscn")
...
func begin_workshop_edit(
	...
	if target_vehicle.get_docked_vehicle_bay() != workshop:
	...
	var lock_result := _session_manager().try_begin(
		self,
		EditorManager.MODE_VEHICLE
	)
	if not bool(lock_result["ok"]):
		return _session_error(str(lock_result["message"]))
...
func _start_workshop_session(
	...
	_apply_turret_transparency(target_vehicle)
	enter_edit_mode()
...
func _get_preview_placement_state() -> int:
	...
	var block_scene := BlockDB.get_scene(selected_block.block_id)
	if block_scene == null or not _can_place_selected_block(block_scene):
		return EditorVisualPolicy.PlacementState.BLOCKED
	var cost := BlockDB.get_construction_cost(selected_block.block_id)
	var missing := ConstructionSupport.get_missing(
		cost,
...
func update_vehicle_removal_hover() -> void:
	...
	for cell: Vector2i in cells:
		centers.append(
			(Vector2(cell) + Vector2(0.5, 0.5)) * Globals.TILE_SIZE
		)
	overlay.show_centers(centers)


func ensure_removal_hover() -> EditorVisualPolicy.RemovalEffect:
...
func place_block() -> void:
	...
	var placed: bool = vehicle.place_block(
		block_scene,
		preview_cell,
		preview_rotation
	)
...
func _apply_turret_transparency(target_vehicle: Vehicle) -> void:
	_restore_turret_transparency()
	for node: Node in get_tree().get_nodes_in_group("turrets"):
		var turret := node as Turret
		if turret == null or not is_instance_valid(turret.mount):
			continue
		var assembly := turret.mount.get_assembly()
		if assembly != null and assembly.host == target_vehicle:
...
func _restore_turret_transparency() -> void:
	EditorVisualPolicy.restore_alpha(transparent_turrets)
