...
var total_mass := 0.0
var weighted_mass_position := Vector2.ZERO
var mass_contributions: Dictionary[Block, Dictionary] = {}
var _bulk_update_depth := 0
var _bulk_connectivity_dirty := false
var _bulk_tracks_dirty := false
var _bulk_power_dirty := false
...
func end_bulk_update() -> void:
	_bulk_update_depth = maxi(_bulk_update_depth - 1, 0)
	if _bulk_update_depth > 0:
		return
	if _bulk_connectivity_dirty or _bulk_tracks_dirty or _bulk_power_dirty:
		var connectivity_dirty := _bulk_connectivity_dirty
		var tracks_dirty := _bulk_tracks_dirty
		var power_dirty := _bulk_power_dirty
		_bulk_connectivity_dirty = false
		_bulk_tracks_dirty = false
		_bulk_power_dirty = false
		update_vehicle(connectivity_dirty, tracks_dirty, power_dirty)


func update_vehicle(
	rebuild_connectivity: bool = true,
	rebuild_tracks: bool = true,
	rebuild_power: bool = true
) -> void:
	if _bulk_update_depth > 0:
		_bulk_connectivity_dirty = _bulk_connectivity_dirty or rebuild_connectivity
		_bulk_tracks_dirty = _bulk_tracks_dirty or rebuild_tracks
		_bulk_power_dirty = _bulk_power_dirty or rebuild_power
		return
	_apply_cached_mass()
	refresh_system_lists()
	if rebuild_connectivity:
...
func refresh_mass_contribution(block: Block) -> void:
	_remove_mass_contribution(block)
	if is_instance_valid(block) and blocks.has(block):
		_add_mass_contribution(block)
	_apply_cached_mass()


func rebuild_mass_cache() -> void:
	total_mass = 0.0
	weighted_mass_position = Vector2.ZERO
	mass_contributions.clear()
	for block: Block in blocks:
		_add_mass_contribution(block)
	_apply_cached_mass()


func _add_mass_contribution(block: Block) -> void:
	if not is_instance_valid(block) or mass_contributions.has(block):
		return
	var contribution_mass := _get_effective_block_mass(block)
	var weighted := block.position * contribution_mass
	mass_contributions[block] = {
		"mass": contribution_mass,
		"weighted": weighted,
	}
	total_mass += contribution_mass
	weighted_mass_position += weighted


func _remove_mass_contribution(block: Block) -> void:
	var contribution: Dictionary = mass_contributions.get(block, {})
	if contribution.is_empty():
		return
	total_mass -= float(contribution.get("mass", 0.0))
	weighted_mass_position -= contribution.get("weighted", Vector2.ZERO)
	mass_contributions.erase(block)


func _get_effective_block_mass(block: Block) -> float:
	var result := block.mass
	if block is TurretMount and is_instance_valid((block as TurretMount).turret):
		result += (block as TurretMount).turret.get_total_mass()
	return result


func _apply_cached_mass() -> void:
	total_mass = maxf(total_mass, 0.0)
	mass = maxf(total_mass, 0.01)
	center_of_mass = (
		weighted_mass_position / total_mass
		if total_mass > 0.0
		else Vector2.ZERO
	)
...
func place_block(
	...
	if update_after_placement:
		update_vehicle(true, block is Track, true)
...
func _merge_union_component(
	...
	for value: Variant in component:
		...
		if (
			...
			leader = candidate
	for value: Variant in component:
		_remove_mass_contribution(value as Block)
...
func destroy_block(block: Block) -> Array[Vehicle]:
	...
	var removed_track := block is Track
	_remove_mass_contribution(block)
	blocks.erase(block)
	...
	update_vehicle(true, removed_track, true)
...
func split_disconnected_components(
	...
	for component_value: Variant in components:
		...
		if component == retained_component:
			continue
		var fragment := (
			load("res://assembly/vehicle/Vehicle.tscn").instantiate()
			as Vehicle
		)
		if fragment == null:
			continue
...
func _move_component_to_vehicle(
	...
	for block_value: Variant in component:
		...
		if component_block == null:
			continue
		_remove_mass_contribution(component_block)
		...
		target_vehicle._add_mass_contribution(component_block)
