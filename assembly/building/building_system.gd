...
var _pending_changed_cells: Dictionary[Vector2i, bool] = {}
var _pending_dirty_chunks: Dictionary[Vector2i, bool] = {}
...
func clear() -> void:
	...
	_pending_changed_cells.clear()
	_pending_dirty_chunks.clear()
	_next_building_id = 1
...
func notify_constructed_changed(
	...
	if _bulk_edit_depth > 0:
		_queue_changed_cells(changed_cells)
		return
...
func _queue_changed_cells(changed_cells: Array[Vector2i]) -> void:
	for cell: Vector2i in changed_cells:
		_pending_changed_cells[cell] = true
		_pending_dirty_chunks[_cell_to_chunk(cell)] = true


func _cell_to_chunk(cell: Vector2i) -> Vector2i:
	var chunk_size := GameMap.MAP_CHUNK_SIZE
	return Vector2i(
...
func _flush_pending_regions() -> void:
	if _pending_changed_cells.is_empty():
		_pending_dirty_chunks.clear()
		return
	var changed_cells: Array[Vector2i] = []
	changed_cells.assign(_pending_changed_cells.keys())
	_pending_changed_cells.clear()
	_pending_dirty_chunks.clear()
	_refresh_changed_region(changed_cells, &"player")
